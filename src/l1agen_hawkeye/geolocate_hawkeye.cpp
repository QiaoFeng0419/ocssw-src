...
int main (int argc, char* argv[])
{
  ...
  int l1a_ndims;
  //int l1a_dimid[32];
  size_t n_SC_rec, sdim, pdim;
  
  enum l1a_grps { scan_attr, telem, navigation, earth_view };
  enum geo_grps { scan_attr_geo, nav_geo, geolocation };
  ...
  status = nc_inq_ndims( l1a_ncid, &l1a_ndims);
  check_err(status,__LINE__,__FILE__);
  for ( int i=0; i<l1a_ndims; i++) {
    //    l1a_dimid[i] = i;
    nc_inq_dimname( l1a_ncid, i, dimname);
    if ( strcmp(dimname, "number_of_SC_records") == 0)
      nc_inq_dimlen( l1a_ncid, i, &n_SC_rec);

    if ( strcmp(dimname, "number_of_scans") == 0)
      nc_inq_dimlen( l1a_ncid, i, &sdim);

    if ( strcmp(dimname, "number_of_pixels") == 0)
      nc_inq_dimlen( l1a_ncid, i, &pdim);
  }

  double *att_time, *orb_time;
  att_time = new double[n_SC_rec];
  orb_time = new double[n_SC_rec];
  
  quat_array *att_quat;
  att_quat = new quat_array[n_SC_rec];

  orb_array *orb_pos, *orb_vel;
  orb_pos = new orb_array[n_SC_rec];
  orb_vel = new orb_array[n_SC_rec];

  double *stime;
  stime = new double[sdim];

  
  // att time
  status = nc_inq_varid	( l1a_gid[navigation], "att_time", &varid);
  check_err(status,__LINE__,__FILE__);

  status = nc_get_var_double( l1a_gid[navigation], varid, att_time);
  check_err(status,__LINE__,__FILE__);

  // att quat
  status = nc_inq_varid	( l1a_gid[navigation], "att_quat", &varid);
  check_err(status,__LINE__,__FILE__);

  status = nc_get_var_float( l1a_gid[navigation], varid, &att_quat[0][0]);
  check_err(status,__LINE__,__FILE__);

  // orb time
  status = nc_inq_varid	( l1a_gid[navigation], "orb_time", &varid);
  check_err(status,__LINE__,__FILE__);

  status = nc_get_var_double( l1a_gid[navigation], varid, orb_time);
  check_err(status,__LINE__,__FILE__);
  
  // orb pos
  status = nc_inq_varid	( l1a_gid[navigation], "orb_pos", &varid);
  check_err(status,__LINE__,__FILE__);

  status = nc_get_var_float( l1a_gid[navigation], varid, &orb_pos[0][0]);
  check_err(status,__LINE__,__FILE__);

  // orb vel
  status = nc_inq_varid	( l1a_gid[navigation], "orb_vel", &varid);
  check_err(status,__LINE__,__FILE__);

  status = nc_get_var_float( l1a_gid[navigation], varid, &orb_vel[0][0]);
  check_err(status,__LINE__,__FILE__);


  // scan_time
  status = nc_inq_varid	( l1a_gid[scan_attr], "scan_time", &varid);
  check_err(status,__LINE__,__FILE__);

  status = nc_get_var_double( l1a_gid[scan_attr], varid, stime);
  check_err(status,__LINE__,__FILE__);
  ...
  quat_array *quatr;
  quatr = new quat_array[n_SC_rec];

  orb_array *posr, *velr;
  posr = new orb_array[n_SC_rec];
  velr = new orb_array[n_SC_rec];

  double omegae = 7.29211585494e-5;
  double ecmat[3][3];

  // Orbit
  for (size_t i=0; i<n_SC_rec; i++) {
    j2000_to_ecr( iyr, iday, orb_time[i], ecmat);

    for (size_t j=0; j<3; j++) {
      posr[i][j] =
        ecmat[j][0]*orb_pos[i][0]+
        ecmat[j][1]*orb_pos[i][1]+
        ecmat[j][2]*orb_pos[i][2];
      velr[i][j] =
        ecmat[j][0]*orb_vel[i][0]+
  ...
  orb_array *posi, *veli, *rpy;
  posi = new orb_array[sdim];
  veli = new orb_array[sdim];
  rpy  = new orb_array[sdim];

  orb_interp( n_SC_rec, sdim, orb_time, posr, velr, stime, posi, veli);

  quat_array *quati;
  quati = new quat_array[sdim];
  q_interp( n_SC_rec, sdim, att_time, quatr, stime, quati);
  //tempOut.open ("qi.bin", ios::out | ios::trunc | ios::binary);
  //tempOut.write((char *) quati, 4*sdim*sizeof(float));
  //tempOut.close();
  ...
  // initialize the quality flags to 0
  bzero(qfl, sdim*pdim);
          
  // Generate pointing vector array
  orb_array *pview;
  pview = new orb_array[pdim];
  for (size_t i=0; i<pdim; i++) {
    double alp = atan((pdim/2 - (i + 41.5))*0.01/focal_length);
    pview[i][0] = 0.0;
    pview[i][1] = sin(alp);
    pview[i][2] = cos(alp);
  }
  ...
  for (size_t i=0; i<3; i++) for (size_t j=0; j<3; j++) sc_to_hwk[i][j] = 0.0;
  sc_to_hwk[0][2] =  1.0;
  sc_to_hwk[1][1] =  1.0;
  sc_to_hwk[2][0] = -1.0;
  ...
  for (size_t i=0; i<sdim; i++) {
    ...
    if ( stime[i] != -999.0) {
      ...
      //    for (size_t j=0; j<3; j++)
      //printf("%le %le %le\n", ptr_C[j*3+0],ptr_C[j*3+1],ptr_C[j*3+2]);
      //printf("\n");
  ...
  status = nc_inq_varid( geo_gid[nav_geo], varname.c_str(), &varid);
  status = nc_put_var_float( geo_gid[nav_geo], varid, (float *) rpy);
  check_err(status,__LINE__,__FILE__);
...
int j2000_to_ecr( int32_t iyr, int32_t idy, double sec, double ecmat[3][3]) {
  ...
  //  double *ptr_C = gsl_matrix_ptr(C,0,0);
  //for (size_t i=0; i<3; i++)
  //  printf("%le %le %le\n", ptr_C[i*3+0],ptr_C[i*3+1],ptr_C[i*3+2]);
  //printf("\n");
  ...
  //for (size_t i=0; i<3; i++)
  //  printf("%le %le %le\n", ptr_E[i*3+0],ptr_E[i*3+1],ptr_E[i*3+2]);
  //printf("\n");
...
int j2000_to_mod( int32_t iyr, int32_t idy, double sec, double j2mod[3][3]) {
  ...
  double zeta0 = ( 2306.2181*t + 0.302*t*t + 0.018*t*t*t)/RADEG/3600;
  double thetap = ( 2004.3109*t - 0.4266*t*t - 0.04160*t*t*t)/RADEG/3600;
  double xip = ( 2306.2181*t + 1.095*t*t + 0.018*t*t*t)/RADEG/3600;
  ...
  if (first) {
    ...
    if ( utcpfile.is_open()) {
      ...
      while ( getline( utcpfile, line)) {
        //        cout << line << '\n';
        istr.clear(); istr.str( line.substr( 0, 5)); istr >> ijd[k];
        istr.clear(); istr.str( line.substr( 44, 9));
        istr >> ut1[k];
        k++;
      }
      ijd[k] = 0;
...
int sun2000( size_t sdim, int32_t iyr, int32_t idy,
  ...
  for (size_t i=0; i<sdim; i++) {
    double t = jday(iyear,1,iday) - (double) 2451545.0 + (sec[i]-43200)/86400;
    ...
    double rs = 1.00014-0.01671*cos(gs/RADEG)-0.00014*cos(2.*gs/RADEG);
...
int uni_geonav( float pos[3], float vel[3], double smat[3][3], double coef[10],
  ...
  for (size_t i=0; i<npix; i++) {
    ...
    float o =
      coef[0]*pview[i][0]*pview[i][0] + coef[1]*pview[i][1]*pview[i][1] +
      coef[2]*pview[i][2]*pview[i][2] + coef[3]*pview[i][0]*pview[i][1] +
      coef[4]*pview[i][0]*pview[i][2] + coef[5]*pview[i][1]*pview[i][2];
    
    float p = coef[6]*pview[i][0] + coef[7]*pview[i][1] + coef[8]*pview[i][2];
    ...
    if ( r >= 0) {
      ...
      solz[i] =
        (short) (100 * RADEG * atan2(sqrt(sl[0]*sl[0] + sl[1]*sl[1]), sl[2]));

      // Check for zenith close to zero
      if ( solz[i] > 0.01)
        sola[i] = (short) (100 * RADEG * atan2(sl[0], sl[1]));

      // Compute the sensor zenith and azimuth
      senz[i] =
        (short) (100 * RADEG * atan2(sqrt(rl[0]*rl[0] + rl[1]*rl[1]), rl[2]));

      // Check for zenith close to zero
      if ( senz[i] > 0.01)
        sena[i] = (short) (100 * RADEG * atan2(rl[0], rl[1]));
...
//  rpy(3)	R*4	 O	Attitude angles (roll, pitch, yaw)
  ...
  // Compute X axis to complete orthonormal triad (yelocity direction)
  double x[3];
  x[0] = y[1] * z[2] - y[2] * z[1];
  x[1] = y[2] * z[0] - y[0] * z[2];
  x[2] = y[0] * z[1] - y[1] * z[0];
  ...
  gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0,
                 &A.matrix, &inv_mat.matrix, 0.0, &rm_mat.matrix);
