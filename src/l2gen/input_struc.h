...
typedef struct input_struct {
    ...
    float add_lt_noise; /* uncertainties, 0=none, !=0 => std dev for gaussian noise */
    float *lt_noise_scale;
    float *bias_frac;
    float add_ws_noise; // wind speed uncertainty, -1=none =0 => use ws_unc as 2*stdev for gaussian noise; >0 => use add_ws_noise as stdev for gaussian noise
    float add_wd_noise; // wind direction uncertainty
    float add_mw_noise; // meridional wind uncertainty
    float add_zw_noise; // zonal wind uncertainty
    float add_pr_noise; // atmospheric pressure uncertainty
    float add_rh_noise; // relative humidity uncertainty
    float add_wv_noise; // water vapor uncertainty
    float add_oz_noise; // ozone uncertainty
    float add_no2_tropo_noise; // tropospheric no2 uncertainty
    float add_no2_strat_noise; // stratospheric no2 uncertainty
