...
int write_l3b_meta_hdf4(int32_t sd_id, meta_l3bType *meta_l3b) {
    ...
    SDsetattr(sd_id, "Data Bins", DFNT_INT64, 1, &meta_l3b->data_bins);
    ...
    SDsetattr(sd_id, "Bin Resolution", DFNT_CHAR,
            strlen(meta_l3b->bin_resolution) + 1, meta_l3b->bin_resolution);
...
int write_l3b_meta_netcdf4(idDS ds_id, meta_l3bType *meta_l3b) {
    ...
    setAttr(ds_id, "geospatial_lon_resolution", NC_DOUBLE, 1,
            (VOIDP) & meta_l3b->geospatial_resolution);

    setAttr(ds_id, "geospatial_lat_resolution", NC_DOUBLE, 1,
            (VOIDP) & meta_l3b->geospatial_resolution);

    strcpy(buf, meta_l3b->bin_resolution);
    setAttr(ds_id, "spatialResolution", NC_CHAR, strlen(buf) + 1, buf);

    int64_t data_bins = meta_l3b->data_bins;
    setAttr(ds_id, "data_bins", NC_INT64, 1, (VOIDP) & data_bins);
