...
namespace units {
    ...
    template <class Y>
    inline constexpr measurement<Y>&
    ...
    measurement<Y>::operator*=(const this_type& source)
    {
        uncertainty_ = (value_ * source.value_) * std::sqrt(sqr(uncertainty_ / value_) + sqr(source.uncertainty_ / source.value_));
        value_ *= source.value_;
    ...
    // stream output
    template <class Y>
    inline std::ostream& operator<<(std::ostream& os, const measurement<Y>& val)
    {
