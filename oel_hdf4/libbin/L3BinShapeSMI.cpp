 ...
 */
int64_t L3BinShapeSMI::getBaseBin(int32_t row) const {
    constrainRow(row);
 ...
 */
int32_t L3BinShapeSMI::bin2row(int64_t bin) {
    if (bin < 1)
        return 0; /* south pole */
    if (bin >= totalBins)
 ...
 */
void L3BinShapeSMI::bin2rowcol(int64_t bin, int32_t &row, int32_t &col) {
    if (bin < 1) {
 ...
 */
int64_t L3BinShapeSMI::rowcol2bin(int32_t row, int32_t col) const {
 ...
 */
void L3BinShapeSMI::rowcol2latlon(int32_t row, int32_t col,
...
int32_t L3BinShapeSMI::lat2row(float lat) const {
    ...
    int32_t row =
            (int32_t) ((lat - south) * (float) totalRows / (north - south));
    if (row >= totalRows)
        row = totalRows - 1;
    if (row < 0)
        row = 0;
 ...
 */
void L3BinShapeSMI::latlon2rowcol(float lat, float lon,
 ...
 */
int64_t L3BinShapeSMI::latlon2bin(float lat, float lon) const {
    int32_t row, col;
 ...
 */
void L3BinShapeSMI::rowcol2bounds(int32_t row, int32_t col,
        float &north, float &south,
