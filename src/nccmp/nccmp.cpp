...
char get_missing(int ncid, varstruct * var, const char* attname) {
    ...
    case NC_UINT:
        ...
        break;
    case NC_INT64:
        status = nc_get_att_long(ncid, var->varid, attname, &var->missing.l);
        if (status != NC_NOERR) return 0;
        break;
    case NC_UINT64:
        status = nc_get_att_long(ncid, var->varid, attname, (long*)&var->missing.ul);
        if (status != NC_NOERR) return 0;
        break;
    case NC_FLOAT:
        ...
        if (verbose) {
            if (vars[i].hasmissing) {
                ...
                case NC_INT: printf("%d (int)\n", vars[i].missing.i);
                    break;
                case NC_UINT: printf("%d (uint)\n", vars[i].missing.ui);
                    break;
                 case NC_INT64: printf("%ld (long)\n", vars[i].missing.l);
                    break;
                case NC_UINT64: printf("%ld (ulong)\n", vars[i].missing.ul);
                    break;
