...
namespace oc {
    /*!
        ...
        \include testdata/liboc/kv_store/test1.par

        A key-value file with groups and another key-value file loaded first (test2.par):
        \include testdata/liboc/kv_store/test2.par

        \section Examples

        Creating a simple map out of key-value files:
        \snippet tests/kvstore.cpp oc::KvStore example1

        Walking though a key-value file as it is being read:
        \snippet tests/kvstore.cpp oc::KvStore example2
    */
    class KvStore {
        using read_callback = std::function<bool(const std::pair<std::string, std::string>&)>;
        using group_filtered_iterator = oc::FilterIterator<std::unordered_map<std::string, std::string>::const_iterator, std::function<bool(const std::pair<std::string, std::string>&)>>;
        using group_iterator = std::vector<std::string>::const_iterator;
        using iterator = std::unordered_map<std::string, std::string>::const_iterator;
        ...
        public:
            ...
            */
            ...
            /*!  \brief oc::FilterIterator iterator to recurse key-values of the input group

                 \return Start iterator
            */
            group_filtered_iterator begin(const std::string& group) const;
            /*!  \brief oc::FilterIterator end iterator to recurse key-values of the input group
