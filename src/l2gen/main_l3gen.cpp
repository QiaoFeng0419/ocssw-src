...
void load_l3file_handle(int sensorID, int filenum, filehandle *file) {
    ...
    file->tot_prod = prodlist(file->sensorID, input->evalmask, file->l2prod,
...
int main(int argc, char* argv[]) {
    ...
    int32 nprods_wt = prodlist(ifile.sensorID, input->evalmask,
    ...
    if (input->suite) {
        const char* doiStr = getDOI(output_binfile->meta_l3b.mission, output_binfile->meta_l3b.sensor, "L3B",
                input->suite,
                input->pversion);
        if (doiStr) {
            strcpy(output_binfile->meta_l3b.doi, doiStr);
        } else {
