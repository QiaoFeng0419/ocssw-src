...
PRIVATE int64_t lastrec[MAX_OUT];
PRIVATE int32_t fstcall[MAX_OUT];
PRIVATE int32_t row[MAX_OUT], rowcount[MAX_OUT];

PRIVATE int64_t *begin[MAX_OUT];
PRIVATE int32_t *extent[MAX_OUT];
PRIVATE int64_t *lastdatabin[MAX_OUT];

PRIVATE int64_t start_num[2160 * 16 + 1];
PRIVATE int64_t lastbin[2160 * 16];
PRIVATE int64_t *numbin;
...
/*------------------------------------------------------------------------------
    ...
    Modification history:
        ...
        Joel Gales       Futuretech      11/24/99    Change parm_opt from
                                                     ...
                                                     in call to buffbins.
------------------------------------------------------------------------------*/
intn put_l3b_record(int32_t prod_ID, int32_t nrec, int64_t *binno,
...
/*------------------------------------------------------------------------------
    ...
    Arguments: (in calling order)
      ...
      int64_t   *lastdatabin  I      ending data bin numbers of each row
