...
int openl1_write(filehandle *l1file) {
    ...
    char *pro_control = input->pro_control;
    char avhrrbird[10];
    ...
    if ((n = rdsensorinfo(l1file->sensorID, input->evalmask, NULL, NULL))
            != numBands) {
        fprintf(stderr, "-E- %s Line %d:  Error reading sensor table. %d %d\n",
                __FILE__, __LINE__, numBands, n);
        return (-1);
    }
    rdsensorinfo(l1file->sensorID, input->evalmask, "Lambda",
            (void **) &Lambda_p);
    rdsensorinfo(l1file->sensorID, input->evalmask, "Fobar",
            (void **) &Fobar_p);
    rdsensorinfo(l1file->sensorID, input->evalmask, "Tau_r",
            (void **) &Tau_r_p);
    rdsensorinfo(l1file->sensorID, input->evalmask, "k_oz",
            (void **) &k_oz_p);
    rdsensorinfo(l1file->sensorID, input->evalmask, "k_no2",
            (void **) &k_no2_p);
    ...
    if (l1file->format == FT_L1HDF) {
        ...
        // TODO: there are prettier ways to do this... a library perhaps?!
		char *env_substitutes[3][2] = {
			{"OCVARROOT", getenv("OCVARROOT")},
			{"OCDATAROOT", getenv("OCDATAROOT")},
			{"OCSSWROOT", getenv("OCSSWROOT")}
		};
		size_t env_substitutes_len[3][2] = {
            {strlen(env_substitutes[0][0]), strlen(env_substitutes[0][1])},
            {strlen(env_substitutes[1][0]), strlen(env_substitutes[1][1])},
            {strlen(env_substitutes[2][0]), strlen(env_substitutes[2][1])}
		};
        char *end_str;
        char *token = strtok_r(input->input_parms, "\n", &end_str);
        while (token != NULL) {
            char *end_token;
            ...
            if (name[0] != '#'){
				...
				for (int i=0;i<3 && !already_added;i++){
					size_t len = strlen(tmp_str);
					if (len > env_substitutes_len[i][1] && strncmp(env_substitutes[i][1], tmp_str, env_substitutes_len[i][1]) == 0){
						char *new_value = malloc(sizeof(char) * (len + strlen("$OCDATAROOT"))); // OCDATAROOT is the longest of the subsitutions
                        new_value[0] = '$';
                        strncpy(new_value + 1, env_substitutes[i][0], env_substitutes_len[i][0]);
                        strncpy(new_value + env_substitutes_len[i][0] + 1, tmp_str + env_substitutes_len[i][1], len - env_substitutes_len[i][1]);
                        new_value[env_substitutes_len[i][0] + (len - env_substitutes_len[i][1]) + 1] = '\0';
                        PTB(SetChrGA(ds_id, name, new_value));
                        free(new_value);
                        already_added = true;
					}
				}
