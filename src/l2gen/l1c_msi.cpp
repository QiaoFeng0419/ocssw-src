...
int inDetector (msi_t *data , float lat, float lon) {
    ...
    tmpX = (double*) allocateMemory(sizeof (double), "tmpX");
    tmpY = (double*) allocateMemory(sizeof (double), "tmpY");
    tmpX[0] = lon * DEG_TO_RAD;
    tmpY[0] = lat * DEG_TO_RAD;
    
    // Project to lon and lat values
    pj_transform(data->pj_latlong, data->pj_utm, 1, 1, tmpX, tmpY, NULL);
    Point_t p(tmpX[0], tmpY[0]);
...
/************************************************/
/*  function: openl1c_msi                       */
/*  Open msi l1c data with metadata file        */
/************************************************/
extern "C" int openl1c_msi(filehandle *file){
     ...
     */
    rdsensorinfo(file->sensorID, input->evalmask, "Fobar", (void **) &Fobar);
...
/************************************************/
/*  function: readl1c_msi                       */
/*  Get coordinates and convert to lon/lat      */
/************************************************/
extern "C" int readl1c_msi_lonlat(filehandle *file, int recnum, l1str *l1rec)
{
    ...
    // Allocate memory
    double* lat = (double *) malloc(file->npix*sizeof(double) );
    double* lon = (double *) malloc(file->npix*sizeof(double) );
    ...
    for (ip=0; ip<file->npix; ip++) {
        lon[ip] = data->ULCoord[0] + 10 + ip * 20;
        lat[ip] = data->ULCoord[1] - 10 - recnum * 20;
    }

    // Project to lon and lat values
    int p = pj_transform(data->pj_utm , data->pj_latlong, file->npix, 1, lon, lat, NULL );
    if(p) {
        fprintf(stderr,"-E- %s line %d: unable to convert MSI UTM meters to lat/long coordinates\n",
            __FILE__,__LINE__);
        return 1;
    }
    ...
    for (ip=0; ip<file->npix; ip++) {
        l1rec->lon[ip] = lon[ip]*RADEG;
        l1rec->lat[ip] = lat[ip]*RADEG;
    }
...
/************************************************/
extern "C" int readl1c_msi(filehandle *file, int recnum, l1str *l1rec, int lonlat)
{
    int i, ip, ib, ipb;
    msi_t* data = (msi_t*) file->private_data; 
    int16_t year, doy;
    ...
    // Get lat lon 
    if(readl1c_msi_lonlat(file,recnum,l1rec)) {
        fprintf(stderr,"-E- %s line %d: unable to allocate lat/lon data for MSI\n",
            __FILE__,__LINE__);
        exit(1);
    }
    ...
    */
    if( ( input->geom_per_band == 1 ) && ( l1rec->geom_per_band == NULL ) )
    {
...
/************************************************/
/*  function: closel1c_msi                      */
/*  Close opened files and free mempry          */
/************************************************/
extern "C" int closel1c_msi(filehandle *file){
