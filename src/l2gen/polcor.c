...
void polcor(l1str *l1rec, int32_t ip) {
    ...
    if (firstCall) {
        ...
        for (ixb = 0; ixb < input->xcal_nwave; ixb++) {
            if ((input->xcal_opt[ixb] & XCALPOL) != 0) {
                if ((ib = bindex_get(input->xcal_wave[ixb])) < 0) {
                    printf("-E- %sline %d: xcal wavelength %f does not match sensor\n",
                            __FILE__, __LINE__, input->xcal_wave[ixb]);
                    exit(1);
                };
                have_xcal[ib] = 1;
            }
        ...
        } else if ((sensorID == VIIRSN) || (sensorID == VIIRSJ1)) {
