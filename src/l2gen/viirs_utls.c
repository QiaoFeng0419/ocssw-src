...
/* ------------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------*/ {
     ...
     */
    printf(
            "%s,%d: An error occurred in getting the leap second\n",
            __FILE__, __LINE__);
    /*
...
/* ------------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------*/ {
    char filename[FILENAME_MAX], line[101], mon_str[5];
    ...
    parse_file_name(input->viirscalparfile, filename);
    if ((fp = fopen(filename, "r")) == NULL) {
        fprintf(stderr,
                "-E- %s line %d: unable to open viirscalparfile: %s for reading\n",
    ...
    while (fgets(line, 100, fp)) {
        trimBlanks(line);
        if((line[0] != '#') && strstr(line, "leapsec_file")) {
            char *val = strchr(line, '=');
            if(val) {
                val++;
                found = 1;
                trimBlanks(val);
                parse_file_name(val, filename);
                break;
            }
        }
    }
    ...
    if (!found) {
        fprintf(stderr,
                "-E- %s line %d: did not find leapsec_file parameter in viirscalparfile: %s\n",
                __FILE__, __LINE__, filename);
        exit(1);
    }
