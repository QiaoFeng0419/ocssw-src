...
int openl1b_viirs_nc(filehandle *l1file) {
    ...
    /*----- Ancillary Data -----*/
    // TODO: remove this?
    rdsensorinfo(l1file->sensorID, input->evalmask,
            "Fobar", (void **) &Fobar);
    ...
    if (*input->calfile != '\0') {
        ...
        load_fcal_lut(input->calfile, (int64_t) grantime, &f_cal_corr);
    }
        ...
        if (bandtype == TEB) {
            for (ipix = 0; ipix < npix; ipix++) {
                ...
                if (strcmp(VARLIST_L1B[iband].name, "M13") == 0) {
                    tmpval = ((uint *) l1b[iband]->data)[ipix];
                } else {
