...
int32 l2_seawifs(filehandle *l1file, filehandle *l2file) {
    PTB(seawifs_meta(l1file, l2file,
            input->sline - 1, input->eline - 1,
            input->dline, input->spixl - 1,
            input->dpixl));
...
int32 seawifs_meta(filehandle *l1file, filehandle *ofile,
    ...
    ds_id.deflate = 0;
    ds_id.fid = sd_id;
    if (ofile->format == FT_L2NCDF || ofile->format == FT_L1BNCDF)
        ds_id.fftype = DS_NCDF;
    else
        ds_id.fftype = DS_HDF;
    ...
    if (ofile->format == FT_L2NCDF) ds_id.fid = ofile->grp_id[1];
    ...
    if (ofile->format == FT_L1HDF || ofile->format == FT_L2HDF) {
        ds_id.fid = ofile->sd_id;
    } else if (ofile->format == FT_L2NCDF) {
        ds_id.fid = ofile->grp_id[4];
    }
