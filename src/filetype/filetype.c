...
file_format chk_olci_xml(char *filename);
file_format chk_msi_xml(char *filename);
...
static file_format chk_hdf5(char *filename) {
    ...
    if(file_id >= 0) {
        ...
        // check for VIIRS SDR
        if(hdf5AttributeReadString(file_id, "Mission_Name", buf)) {
...
file_format getFormat(char *filename) {
    ...
    if (sd_id != FAIL) {
        ...
        if (ds_id.fid == FAIL && (ret = chk_olci_xml(filename)).type != FT_INVALID) {
            if (want_verbose) {
        ...
        /* Check format of MSI data with chk_msi function */
        if ((ret = chk_msi_xml(filename)).type != FT_INVALID) {
            if (want_verbose) {
                printf("Input file %s is a valid Sentinel 2A MSI file.\n", filename);
            }
            return ret;
        }
    }
