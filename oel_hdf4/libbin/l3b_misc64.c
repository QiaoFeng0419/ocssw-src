...
#include <stdlib.h>
#include "seabin64.h"
#include "seaprotoi64.h"

/*  bin counters  */
intn count[MAX_OUT];
int64_t total[MAX_OUT];

/*  static buffers  */
static uchar8 mstrbuff[MAX_OUT][NBINS * MSTRSIZE];
static uchar8 slvbuff[MAX_OUT][NPARMS][NBINS * SLVSIZE];

extern int32_t NUMROWS;
extern int64_t TOTBINS;

/*------------------------------------------------------------------------------
    Function: setupgrid

    Returns: int32_t (ID of grid Vgroup)
    ...
    Arguments: (in calling order)
      ...
      ----      ----        ---   -----------
      int32_t   prod_ID     I     ID number of product
      int32_t   fid         I     HDF file ID
...
int32_t
#ifdef PROTOTYPE
setupgrid(int32_t prod_ID, int32_t fid)
#else
...
/*------------------------------------------------------------------------------
    ...
    Returns: int32_t (number of bins written to grid)
    ...
    Arguments: (in calling order)
      ...
      ----      ----        ---   -----------
      int32_t   prod_ID     I     ID number of product
      int32_t   gridid      I     HDF Vgroup ID of grid
...
int32_t
#ifdef PROTOTYPE
closegrid(int32_t prod_ID, int32_t gridid)
#else
closegrid(prod_ID, gridid)
int32_t prod_ID, gridid;
#endif
{
    Vdetach(gridid);
...
/*------------------------------------------------------------------------------
    ...
    Returns: int32_t (ID number of geometry Vdata)
...
int32_t writegeom(int32_t fid, int32_t nbins) {
    ...
    i32 = CENTER;
    memcpy(&dummy[0], &i32, sizeof (int32_t));
    i32 = NO;
    memcpy(&dummy[4], &i32, sizeof (int32_t));
    i32 = nbins;
    memcpy(&dummy[8], &i32, sizeof (int32_t));
    f64 = RADIUS;
    memcpy(&dummy[12], &f64, sizeof (double));
    f64 = MAXNORTH;
    memcpy(&dummy[20], &f64, sizeof (double));
    f64 = MAXSOUTH;
    memcpy(&dummy[28], &f64, sizeof (double));
    f64 = SEAMLON;
    memcpy(&dummy[36], &f64, sizeof (double));
...
/*------------------------------------------------------------------------------
    ...
    Returns: int32_t (ID number of Index Vdata)
...
/*------------------------------------------------------------------------------
    ...
    Arguments: (in calling order)
      Type      Name        I/O   Description
      ----      ----        ---   -----------
      int32_t   fid         I     HDF Vdata ID
      int32_t * start_num   I     1-D array - # of first bin in each row
      int32_t * begin       I     1-D array - # of first data bin in each row
      int32_t * extent      I     1-D array - # of bins in each row
      int32_t * maxbin      I     1-D array - max # of bins in each row
...
intn
#ifdef PROTOTYPE
writeindex(int32_t ndxid, int64_t *start_num, int64_t *begin, int32_t *extent,
        int64_t *maxbin)
#else
writeindex(ndxid, start_num, begin, extent, maxbin)
int32_t ndxid, *extent;
int64_t *start_num, *begin, *maxbin;
#endif
{
    uchar8 *buffer, *b;
    double vs, hs;
    ...
    for (row = 0; row < NUMROWS; row++) {
        memcpy(b, &row, sizeof (int32_t));
        b += sizeof (int32_t);
        vs = (double) 180.0 / (double) (NUMROWS / 1.0);
        memcpy(b, &vs, sizeof (double));
        b += sizeof (double);
        hs = (double) 360.0 / (double) (maxbin[row + 1]);
        memcpy(b, &hs, sizeof (double));
        b += sizeof (double);
        memcpy(b, &start_num[row + 1], sizeof (int32_t));
        b += sizeof (int32_t);
        memcpy(b, &begin[row + 1], sizeof (int32_t));
        b += sizeof (int32_t);
        memcpy(b, &extent[row + 1], sizeof (int32_t));
        b += sizeof (int32_t);
        memcpy(b, &maxbin[row + 1], sizeof (int32_t));
        b += sizeof (int32_t);
...
/*------------------------------------------------------------------------------
    ...
    Returns: int32_t (ID number of DataMaster Vdata)
    ...
    Arguments: (in calling order)
      ...
      ----      ----        ---   -----------
      int32_t   fid         I     HDF file ID
      int32_t   fsize       I     type number for output product
...
int32_t
#ifdef PROTOTYPE
setupmaster(int32_t fid, int32_t fsize)
#else
setupmaster(fid, fsize)
int32_t fid, fsize;
#endif
{
    int32_t mstrid, ref, stat;
    uchar8 *buffer;
...
/*------------------------------------------------------------------------------
    ...
    Returns: int32_t * (array of ID numbers of DataSlave Vdatas)
    ...
    Arguments: (in calling order)
      ...
      int32_t * parm_opt    I     option array of parameters to be written
...
int32_t *
#ifdef PROTOTYPE
setupslaves(int32_t fid, int32_t fsize, char *l3b_path, l3b_prod *parm_opt)
#else
...
#endif
{
    ...
    slvid = (int32_t *) malloc(NPARMS * sizeof (int32_t));
    for (i = 0; i < NPARMS; i++) {
    ...
    for (i = 0; i < NPARMS; i++) {
        ...
        if (parm_opt->code[i] == 1) {
            ...
            if (fsize == SCENE) {
                ...
                aid = HXcreate(fid, DFTAG_VS, (uint16_t) slvref, fname, 512, 0);
                Hendaccess(aid);
            }
            VSdetach(slvid[i]);
...
/*------------------------------------------------------------------------------
    ...
    Arguments: (in calling order)
      ...
      ----      ----        ---   -----------
      int32_t   prod_ID     I     ID number of product
      int32_t   mstrid      I     ID number of master Vdata
      int32_t * slvid       I     array of IDs of slave Vdatas
      int32_t * parm_opt    I     option array of parameters to be written
...
intn
#ifdef PROTOTYPE
closedata(int32_t prod_ID, int32_t mstrid, int32_t *slvid, l3b_prod *parm_opt)
#else
closedata(prod_ID, mstrid, slvid, parm_opt)
int32_t prod_ID, mstrid, *slvid;
l3b_prod *parm_opt;
#endif
{
    intn i;
...
/*------------------------------------------------------------------------------
    ...
    Arguments: (in calling order)
      ...
      int32_t   prod_ID     I     ID number of product
      int32_t   mstrid      I     ID number of master Vdata
      int32_t * slvid       I     array of ID numbers of slave Vdatas
      int32_t   nrec        I     number of bins being buffered
      int32_t * binno       I     bin number for each bin
      int16_t * nobs        I     # of observations contributing to each bin
      int32_t * ttag        I     time record for each bin
      int16_t * nscenes     I     # of scenes contributing for each bin
      float *   weights     I     statistical weight for each bin
      int8_t  * sel_cat     I     selection category for each binno list
      int16_t * flags_set   I     flags corresponding to parent l2_flags
      float *   l3b_data    I     3-D array of sums and sum-squares
      int32_t * parm_opt    I     option array of parameters to be written
...
intn
#ifdef PROTOTYPE
buffbins(int32_t *fstcall, int32_t prod_ID, int32_t mstrid, int32_t *slvid,
        int32_t nrec, int32_t *binno, int16_t *nobs, int16_t *ttag, int16_t *nscenes,
...
int32_t *fstcall, prod_ID, mstrid, *slvid, nrec, *flags_set;
int64_t *binno;
int16_t *nobs, *nscenes, *ttag;
int8_t *sel_cat;
float *weights, *l3b_data;
l3b_prod *parm_opt;
#endif
{
    ...
    static uchar8 * bn[MAX_OUT], *ns[MAX_OUT], *nsc[MAX_OUT],
            ...
            *fs[MAX_OUT]; /*LK*/
    static float *x0[MAX_OUT][NPARMS], *d0[MAX_OUT][NPARMS];

    static int32_t slvincr = NPARMS * 2;
    static int32_t mstrincr = MSTRSIZE;
    ...
    if (first) {
        ...
        for (i = 0; i < MAX_OUT; i++) {
            ...
            for (j = 0; j < NPARMS; j++) {
                if (parm_opt->code[j] == 1) {
                    x0[i][j] = (float *) &slvbuff[i][j][0];
                    d0[i][j] = (float *) &l3b_data[j * 2];
                }
            }
        }
        first = 0;
        ...
        for (j = 0; j < NPARMS; j++) {
            if (parm_opt->code[j] == 1)
                d0[prod_ID][j] = (float *) &l3b_data[j * 2];
        }
    }
    if (fstcall[prod_ID]) { /*LK*/
        ...
        for (j = 0; j < NPARMS; j++) { /*LK*/
            if (parm_opt->code[j] == 1) /*GF*/ {
                x0[prod_ID][j] = (float *) &slvbuff[prod_ID][j][0]; /*LK*/
                d0[prod_ID][j] = (float *) &l3b_data[j * 2]; /*LK*/
            } /*GF*/
        } /*LK*/
    ...
    for (i = 0; i < nrec; i++) {
        if ((binno[i] > 0) && (binno[i] < TOTBINS)) {
            ...
            if (count[prod_ID] == NBINS) {
                ...
                for (j = 0; j < NPARMS; j++) {
                    if (parm_opt->code[j] == 1)
                        x0[prod_ID][j] = (float *) &slvbuff[prod_ID][j][0];
                }
                flushbuff(prod_ID, mstrid, slvid, parm_opt);
                count[prod_ID] = 0;
            } else {
...
intn
#ifdef PROTOTYPE
flushbuff(int32_t prod_ID, int32_t mstrid, int32_t *slvid, l3b_prod *parm_opt)
#else
flushbuff(prod_ID, mstrid, slvid, parm_opt)
int32_t prod_ID, mstrid, *slvid;
l3b_prod *parm_opt;
#endif
{
    ...
    if (count[prod_ID] > 0) {
        ...
        for (i = 0; i < NPARMS; i++) {
            if (parm_opt->code[i] == 1) {
                stat = VSwrite(slvid[i], &slvbuff[prod_ID][i][0],
                        count[prod_ID], FULL_INTERLACE);
...
/*------------------------------------------------------------------------------
    ...
    Returns: int32_t (ID number of attribute Vdata)
    ...
    Arguments: (in calling order)
      ...
      int32_t   nt          I     HDF number type constant
...
int32_t
#ifdef PROTOTYPE
writeattr(int32_t fid, char *name, int32_t nt, intn count, VOIDP data)
#else
