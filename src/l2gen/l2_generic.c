...
int openl2(filehandle *l2file) {
    ...
    if (l2file->format == FT_L2HDF) {
      ...
      tot_prod = prodlist(l2file->sensorID,input->evalmask,l2file->l2prod, l2file->def_l2prod,l2file->l2_prod_names);
    ...
    rdsensorinfo(l2file->sensorID, input->evalmask, "Bindx", (void **) &l2file->bindx);

    /* set wavelength index */
    rdsensorinfo(l2file->sensorID, input->evalmask, "Lambda", (void **) &l2file->iwave);
    rdsensorinfo(l2file->sensorID, input->evalmask, "fwave", (void **) &l2file->fwave);
    rdsensorinfo(l2file->sensorID, input->evalmask, "Fobar", (void **) &l2file->Fobar);
    rdsensorinfo(l2file->sensorID, input->evalmask, "Tau_r", (void **) &l2file->Tau_r);
    rdsensorinfo(l2file->sensorID, input->evalmask, "k_oz", (void **) &l2file->k_oz);
    rdsensorinfo(l2file->sensorID, input->evalmask, "k_no2", (void **) &l2file->k_no2);
    ...
    /* ---------------------------------------------------------------- */
    /*                                                                  */
    tot_prod = prodlist(l2file->sensorID, input->evalmask, l2file->l2prod,
    ...
    for (i = 0; i < tot_prod; i++) {
        ...
        if( strcmp( l2file->productInfos[i]->category, "Anc_profile" ) ) {
            dm[2] = numBands;
            strcpy((char *) dm_name[2], bandsPerPixelStr);
        } else {
            dm[2] = numLvlProf;
    ...
    PTB(SetChrGA(ds_id, processingVersionStr, input->pversion));
    ...
    PTB(SetChrGA(ds_id, inputFilesStr, input->input_files));
    ...
    if (l2file->format == FT_L2NCDF) {
        ...
        // create the id -
        if (strcmp(input->pversion, "Unspecified") != 0) {
            strcpy(buf1, input->pversion);
            strcat(buf1, "/L2/");
        } else {
            strcpy(buf1, "L2/");
        }
        ...
        const char* doiStr = getDOI(sensorId2PlatformName(l2file->sensorID), sensorId2InstrumentName(l2file->sensorID),
                "L2", input->suite, input->pversion);
        if (doiStr) {
            PTB(SetChrGA(ds_id, "identifier_product_doi_authority", "http://dx.doi.org"));
            PTB(SetChrGA(ds_id, "identifier_product_doi", doiStr));
        }
        ...
        char *tmp_parms = replace_ocroots(input->input_parms);
        ...
        PTB(SetChrGA(ds_id, "Input Parameters", input->input_parms));
    ...
    if (numBands > 0) {
        PTB(writeDS(ds_id, "vcal_gain", input->gain, 0, 0, 0, numBands, 0, 0));
        PTB(writeDS(ds_id, "vcal_offset", input->offset, 0, 0, 0, numBands, 0, 0));
...
int writel2(filehandle *l2file, int32_t recnum, l2str *l2rec, int outfile_number) {
    ...
    idDS ds_id;
    static int32_t l2_flag_cnt[NFLAGS];
    ...
    PTB(writeDS(ds_id, "slon", &(l2rec->l1rec->lon[spix]), recnum, 0, 0, 1, 1, 1));
    PTB(writeDS(ds_id, "clon", &(l2rec->l1rec->lon[cpix]), recnum, 0, 0, 1, 1, 1));
    PTB(writeDS(ds_id, "elon", &(l2rec->l1rec->lon[epix]), recnum, 0, 0, 1, 1, 1));
    PTB(writeDS(ds_id, "slat", &(l2rec->l1rec->lat[spix]), recnum, 0, 0, 1, 1, 1));
    PTB(writeDS(ds_id, "clat", &(l2rec->l1rec->lat[cpix]), recnum, 0, 0, 1, 1, 1));
    PTB(writeDS(ds_id, "elat", &(l2rec->l1rec->lat[epix]), recnum, 0, 0, 1, 1, 1));
    ...
    if (outfile_number == 0) {
        if ((first == 1) || (fabs(last_lat - l2rec->l1rec->lat[cpix]) > GEOBOX_INC)
            ...
            if (!(l2rec->l1rec->flags[cpix] & NAVFAIL)) {
                ...
                if ((!(l2rec->l1rec->flags[spix] & NAVFAIL)
                        && !(l2rec->l1rec->flags[epix] & NAVFAIL))) {

                    first = 0;
                    geobox[0][geobox_cnt] = l2rec->l1rec->lon[spix];
                    geobox[1][geobox_cnt] = l2rec->l1rec->lat[spix];
                    geobox[2][geobox_cnt] = l2rec->l1rec->lon[epix];
                    geobox[3][geobox_cnt] = l2rec->l1rec->lat[epix];
    ...
    for (i = 0; i < l2file->tot_prod; i++) {
        ...
        if (p->rank == 3) {
            int32_t num_3d;
            /*  to use another 3rd dim for the anc profiles  */
            num_3d = ( strcmp( l2file->productInfos[i]->category, "Anc_profile" ) ) ?
                numBands : numLvlProf;
    ...
    update_flag_cnts(l2_flag_cnt, l2rec->l1rec->flags, NFLAGS, l2rec->l1rec->npix, 1L);
    ...
    if (recnum == (numScans - 1)) {
        ...
        if (l2file->format == FT_L2NCDF) {
            ...
            write_flag_pcnts(ds_id, fp_meta, l2_flag_cnt, NFLAGS, l2_flag_lname, numScans, numPixels);
