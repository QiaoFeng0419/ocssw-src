 ...
 *******************************************************************/ {
    ...
    if (l1rec->l1file->format == FT_OLCI) {
        olci_data = (olci_t *) (l1rec->l1file->private_data);
        file_olci = olci_data->tieMeteoFile;
    }
            ...
            switch (iprm) {
                ...
                case MW: /* the mwind comes second, so then combinations cn be done */
                    ...
                    if ((uwnd + vwnd) > 0.05 * (u_unc + v_unc)) {
                        l1rec->ws_unc[ilon] = sqrt((uwnd * uwnd * u_unc * u_unc +
                                vwnd * vwnd * v_unc * v_unc) / ws_2);
                        l1rec->wd_unc[ilon] = sqrt(vwnd * vwnd * u_unc * u_unc +
                                uwnd * uwnd * v_unc * v_unc) / ws_2;
                        if (l1rec->wd_unc[ilon] > M_PI) l1rec->wd_unc[ilon] = M_PI;
                    } else {
                        l1rec->ws_unc[ilon] = sqrt(0.5 * (u_unc * u_unc + v_unc * v_unc));
                        l1rec->wd_unc[ilon] = M_PI;
                    }
                    l1rec->wd_unc[ilon] *= r2d;
                    break;
                case PR:
 ...
 *******************************************************************/ {
    ...
    int32_t n_met = 4; /* # met parms finally needed */
    ...
    for (iprm = 0; iprm < n_raw_gmao; iprm++) {
        ...
        switch (iprm) {
            ...
            case PR:
                ...
                break;
            case RH:
                if (anc_acq_read_gmao(file, "met", "T10M", &data2, &qual, &time,
                        &nlon, &nlat, &nlvl, &lon_coord, &lat_coord) != 0)
                    return 1;
                if (anc_acq_read_gmao(file, "met", "QV10M", &data3, &qual, &time,
                        &nlon, &nlat, &nlvl, &lon_coord, &lat_coord) != 0)
                    return 1;
 ...
 *******************************************************************/ {
    /* list of GMAO groups, parm names and special processing value */
    /* note that for the second RH, use the the 1st RH */
    char *ob_gmao_prm_nm[] = {"T", "RH", "H", "QV"};
    int32_t n_raw_gmao = 4;
 ...
 *******************************************************************/ {
    ...
    *nlon = nlon_pre + 1;
    ...
    time[0] = isodate2unix((const char*) isodate);
    ...
    if ((nc_inq_varid(ncid, ds_name, &var_id) != NC_NOERR) ||
        ...
        fprintf(stderr,
                "-E- %s %d: file: %s error setting an id\n",
                __FILE__, __LINE__, file);
        return 1;
    }
