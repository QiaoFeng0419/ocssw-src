...
int main (int argc, char* argv[])
{
  ...
  if ( argc == 2) {
    ...
    hebFile.write((char *) heb_buf.data(), heb_buf.size());
    hebFile.close();
  ...
  for (size_t i=0; i<nTlmBlocks; i++) {
    ...
    for (size_t j=0; j<nChannels; j++) {
      ...
      case TC_CCD4_TEMP:
        if ( interp == 1)
          ccd_temperatures[i*nCCDtemps+(j-TC_CCD1_TEMP)] = value / 32.0;
        else
  ...
  ////////////////////// Write EV bands //////////////////////
  double scan_time[6000];
  int32_t delta_time[6000];
  ...
  uint16_t finderHeight = streamInfo.finderscopeInfo[0].height;
  uint16_t finderWidth =  streamInfo.finderscopeInfo[0].width;
  ...
  if (ngood > nSC) ngood = nSC;
  if (n[SENSOR] == 0) {
    ngood = 0;  // Set to 0 if no sensor records
