...
int run_soa_sma(l2str *l2rec, int32_t ip) {
    ...
    float adg_s = input->gsm_adg_s;
    float bbp_s = input->gsm_bbp_s;
    ...
    float *t_sol = &l1rec->t_sol [ip * nwave];
    float *t_sen = &l1rec->t_sen [ip * nwave];
    float *TLg = &l1rec->TLg [ip * nwave];
    float *tg_sol = &l1rec->tg_sol[ip * nwave];
    ...
    for (ib = 0; ib < nwave; ib++) {
        ...
        Ltemp[ib] -= l1rec->TLg[ipb];

        /*  Solar irradiance and Rayleigh reflectance */
        Rhor_in[ib] = pi * l1rec->Lr[ipb] / Fo[ib] / mu0;
    }


    if (input->aer_opt == AERSOA) {

        atmcor_soa_(
                &(l1file->sensorID),
                (char*)sensorId2SensorDir(l1file->sensorID),
                &nwave,
                wave,
                &(l1rec->iscan),
                &ip,
...
void get_soa(l2str *l2rec, int32_t prodID, float prod[]) {
    ...
    for (ip = 1; ip <= l2rec->l1rec->npix; ip++)
        if (l2rec->l1rec->mask[ip - 1] == 0)
