...
int openl1_hmodis_hdf(filehandle *l1file) {
    ...
    if (init_l1b(l1file->name, &input->resolution))
        exit(EXIT_FAILURE);
    ...
    rdsensorinfo(l1file->sensorID, input->evalmask, "Fobar", (void **) &Fobar);
...
int readl1_hmodis_hdf(filehandle *l1file,
    ...
    if (iscan != prev_scan) {
        prev_scan = iscan;
        load_modis_scan(iscan, input->resolution, l1file->sensorID);
    }
            ...
            for (ip = 0; ip < l1rec->npix; ip++) {
                ...
                if (SI_val < MIN_BAD_SI) {
                    ...
                    if (l1rec->l1file->sensorID == MODIST) {
                        ...
                        if (ical == 1)
                            value /= input->ch22detcor[line % 10];
                        /* channel 23 detector balancing */
                        if (ical == 2)
                            value /= input->ch23detcor[line % 10];
                    }
                }
    ...
    radiance2bt(l1rec, input->resolution);
...
int readl1_lonlat_hmodis_hdf(filehandle *l1file, int32_t line, l1str *l1rec) {
    ...
    int32_t resolution = input->resolution;
