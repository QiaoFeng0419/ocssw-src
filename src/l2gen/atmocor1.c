...
void atmocor1(l1str *l1rec, int32_t ip) {
    ...
    int32_t *wave = l1rec->l1file->iwave;
    float *Fo = l1rec->Fo;
...
-- bryan
    ...
    if ((input->gas_opt & ATREM_BIT) != 0) {
        ...
        get_atrem_cor(sensorID, l1rec, ip, rhot, tg_tot, &l1rec->tg_sol[ip * nwave], &l1rec->tg_sen[ip * nwave]);
    ...
    //input->gas_opt,sensorID,input->evalmask,nwave,mu0,mu,oz,wv,no2_tropo,no2_strat, no2_frac,
