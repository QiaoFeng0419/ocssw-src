...
float calcite_3b(l2str *l2rec, int32_t ip) {
    ...
    bbw546 = seawater_bb(546.0, l1rec->sstref[ip], l1rec->sssref[ip]);
    ...
    if (l1file->sensorID == SEAWIFS) {
        for (ib = 0; ib < nwave; ib++) {
            ...
            rhoaw[ib] = ((l2rec->l1rec->Lt[ipb] / l1rec->tg_sol[ipb] / l1rec->tg_sen[ipb] - l1rec->tLf[ipb]
                    - l1rec->Lr[ipb]) / l1rec->t_o2[ipb] - l1rec->TLg[ipb]) * pi / l1rec->Fo[ib] / mu0;
        }
        for (i = 0; i < 3; i++) {
            rho[i] = rhoaw[bx[i]];
            for (ib = 0; ib < nwave; ib++) {
...
float calcite_2b(l2str *l2rec, int32_t ip) {
    ...
    if (bandShift) {
        float Rrs555 = conv_rrs_to_555(l2rec->Rrs[ip * nwave + ib550], l1file->fwave[ib550]);
...
float calcite_c(l2str *l2rec, int32_t ip) {
    ...
    int32_t ib443 = windex(443., l2rec->l1rec->l1file->fwave, nwave);
    int32_t ib550 = windex(550., l2rec->l1rec->l1file->fwave, nwave);
...
float calcite_ci2(l2str *l2rec, int32_t ip) {
    ...
    if (RrsRed >= 0.0 && RrsGreen >= 0.0) {
        caco3 = 0.4579 * (RrsGreen - RrsRed) - 0.0006;
    }
    return (caco3);
}
