...
PRIVATE int32_t sdfid[MAX_OUT], fid[MAX_OUT], geomid[MAX_OUT], ndxid[MAX_OUT];
PRIVATE int32_t gridid[MAX_OUT], mstrid[MAX_OUT], *slvidp[MAX_OUT];
PRIVATE int32_t continuity[MAX_OUT];
PRIVATE int32_t lastrec[MAX_OUT];
PRIVATE int32_t fstcall[MAX_OUT];
PRIVATE int32_t row[MAX_OUT], rowcount[MAX_OUT];

PRIVATE int32_t *begin[MAX_OUT];
PRIVATE int32_t *extent[MAX_OUT];
PRIVATE int32_t *lastdatabin[MAX_OUT];

PRIVATE int32_t start_num[2160 * 16 + 1];
PRIVATE int32_t lastbin[2160 * 16];
PRIVATE int32_t *numbin;

extern int32_t NUMROWS;
extern int32_t TOTBINS;

/*, numbin[NUMROWS+1];*/
/*PRIVATE double latbin[NUMROWS+1];*/
...
/*------------------------------------------------------------------------------
    ...
    Returns: int32_t (ID number of product file)
    ...
    Arguments: (in calling order)
      ...
      int32_t   fsize       I     product type (SCENE, DAILY,...)
...
int32_t put_l3b_open(char *l3b_path, char *replaces, int32_t fsize,
    ...
    begin[prod_ID] = calloc(NUMROWS + 1, sizeof (int32_t));
    extent[prod_ID] = calloc(NUMROWS + 1, sizeof (int32_t));
    lastdatabin[prod_ID] = calloc(NUMROWS + 1, sizeof (int32_t));
    ...
    SDsetattr(sdfid[prod_ID], SENSOR_NAME, DFNT_CHAR,
            strlen(meta_l3b->sensor_name) + 1, meta_l3b->sensor_name);

    SDsetattr(sdfid[prod_ID], "project", DFNT_CHAR,
            strlen(DCENTER_VAL) + 1, DCENTER_VAL);

    if (meta_l3b->mission != 0x0) {
        SDsetattr(sdfid[prod_ID], "mission", DFNT_CHAR,
    ...
    geomid[prod_ID] = writegeom(fid[prod_ID], (int32_t) 2 * NUMROWS);
    ...
    for (i = 0; i < NPARMS; i++)
        if (parm_opt->code[i] == 1)
            Vaddtagref(gridid[prod_ID], DFTAG_VH,
                VSQueryref(slvidp[prod_ID][i]));
...
/*------------------------------------------------------------------------------
    ...
    Arguments: (in calling order)
      Type         Name      I/O     Description
      ----         ----      ---     -----------
      int32_t      prod_ID   I       id of file to write to
      int32_t      nrec      I       number or bins to write
      int32_t *    binno     I       bin number for each bin
      int16_t *    nobs      I       # of observations contributing to each bin
      int16_t *    time_rec  I       time record for each bin
      int16_t *    nscenes   I       # of scene contributing to each bin
      float *      weights   I       statistical weight for each bin
      int8_t *     sel_cat   I       selection category for eh binno bin
    ...
    Modification history:
        ...
        Joel Gales       Futuretech      11/24/99    Change parm_opt from
                                                     ...
                                                     in call to buffbins.
------------------------------------------------------------------------------*/
intn put_l3b_record(int32_t prod_ID, int32_t nrec, int32_t *binno,
    ...
    for (i = 0; i < nrec; i++) {
        if ((binno[i] > TOTBINS) || (binno[i] < 1)) {
            ...
            fprintf(stderr, "Bin %ld skipped on prod_ID %d\n",
                    (long int) binno[i], prod_ID);
        }
        if (binno[i] < lastrec[prod_ID]) {
            fprintf(stderr,
                    "*** Error:  Bin numbers not in ascending order.\n");
            fprintf(stderr, "*** calling bin#%ld after bin#%ld\n",
                    (long int) binno[i], (long int) lastrec[prod_ID]);
...
/*------------------------------------------------------------------------------
    ...
    Arguments: (in calling order)
      Type         Name      I/O     Description
      ----         ----      ---     -----------
      int32_t      prod_ID   I       id of file to close
      int16_t      bin_syear I       start year for binning period
      int16_t      bin_sday  I       start day-of-year for binning period
      int16_t      bin_eyear I       end year for binning period
      int16_t      bin_eday  I       end day-of-year for binning period
      int16_t      syear     I       data start time(year)
      int16_t      sday      I       data start time(day)
      int32_t      smsec     I       data start time(msec)
      int16_t      eyear     I       data end time(year)
      int16_t      eday      I       data end time(day)
      int32_t      emsec     I       data end time(msec)
      ...
      uint8_t *    eng_q_use I       values for exclusion during binning
...
intn put_l3b_close(int32_t prod_ID, int16_t bin_syear, int16_t bin_sday,
    ...
    SDsetattr(sdfid[prod_ID], "time_coverage_start", DFNT_CHAR,
    ...
    sprintf(string, "%4.4d%3.3d%2.2d%2.2d%2.2d%3.3d", eyear, eday,
            quot1.quot, quot2.quot, quot3.quot, quot3.rem);
    SDsetattr(sdfid[prod_ID], "time_coverage_end", DFNT_CHAR,
            strlen(string) + 1, (VOIDP) string);
...
/*------------------------------------------------------------------------------
    ...
    Arguments: (in calling order)
      Type         Name      I/O     Description
      ----         ----      ---     -----------
      int32_t   *begin        I      starting data bin numbers of each row
      int32_t   *lastdatabin  I      ending data bin numbers of each row
      int32_t   *lastrec      I      ending data bin number of last data row
      float     *nlat         O      northernmost latitude
      float     *slat         O      southernmost latitude
      float     *elon         O      easternmost longitude
      float     *wlon         O      westernmost longitude
...
-----------------------------------------------------------------------------*/
int32_t get_coords(int32_t *begin, int32_t *lastdatabin, int32_t lastrec,
