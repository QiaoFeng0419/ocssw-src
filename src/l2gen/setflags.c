...
void l1_mask_set(l1str *l1rec, int32_t ip) {
    l1rec->mask[ip] = (input->landmask && l1rec->land [ip]) ||
            (input->bathmask && l1rec->swater [ip]) ||
            (input->cloudmask && l1rec->cloud [ip]) ||
            (input->glintmask && l1rec->glint [ip]) ||
            (input->stlightmask && l1rec->stlight[ip]) ||
            (input->satzenmask && l1rec->senzmax[ip]) ||
            (input->sunzenmask && l1rec->solzmax[ip]) ||
            (input->hiltmask && l1rec->hilt [ip]) ||
            (l1rec->filter[ip]) ||
            (l1rec->navfail[ip]);
}
...
void setflagbits(int level, l1str *l1rec, l2str *l2rec, int32_t ipix) {
    ...
    if (level == 0) {
        ...
        for (ip = spix; ip <= epix; ip++) {
            ...
            for (iw = 0; iw < nwave; iw++) {
                ib = iw;
                if (l1rec->dpol[ip * nwave + ib] > input->hipol) {
