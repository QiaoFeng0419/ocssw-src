...
void gringHelper::process_scan(l1str *l1rec, size_t recnum, size_t escan) {
    ...
    if((gring_test_val) && (geobox_cnt < MAX_GEOBOX_CNT - 1)) {
        ...
        for (geobox_num=0; geobox_num<(int)geobox_cnt; geobox_num++) {
            //printf("INFO: process_scan: geobox scan line from (%f,%f) to (%f,%f)\n",
            //        geobox[1][geobox_num],geobox[0][geobox_num],
            //        geobox[3][geobox_num],geobox[2][geobox_num]);
        }
    }
...
void gringHelper::createGring(gring_t *gringOut) {
    ...
    float **geoboxOrdered;
    size_t i,j,geobox_num;
    
    // Allocate memory for geoboxOrdered[4][MAX_GEOBOX_CNT]
    geoboxOrdered = allocate2d_float(4, MAX_GEOBOX_CNT);

    for (geobox_num=0; geobox_num<geobox_cnt; geobox_num++) {
        //printf("INFO: createGring: geobox scan line from (%f,%f) to (%f,%f)\n",
        //            geobox[1][geobox_num],geobox[0][geobox_num],
        //            geobox[3][geobox_num],geobox[2][geobox_num]);
    }
    ...
    for (geobox_num=0; geobox_num<geobox_cnt; geobox_num++) {
        //printf("INFO: createGring: geoboxOrdered scan line from (%f,%f) to (%f,%f)\n",
        //            geoboxOrdered[1][geobox_num],geoboxOrdered[0][geobox_num],
        //            geoboxOrdered[3][geobox_num],geoboxOrdered[2][geobox_num]);
    }
