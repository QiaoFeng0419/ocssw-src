...
#include "seabin64.h"
#include "seaproto64.h"
#include "getl3b64.h"

#define MAXNAMELEN  255

PRIVATE float *p[MAX_IN][NPARMS];
PRIVATE int32_t *binno_list[MAX_IN];
PRIVATE int16_t *nobs_list[MAX_IN], *nscenes_list[MAX_IN];
PRIVATE int16_t *timerec_list[MAX_IN];
PRIVATE int8_t *selcat_list[MAX_IN];
PRIVATE int32_t *flags_list[MAX_IN];
PRIVATE float *weights_list[MAX_IN];
...
/*-----------------------------------------------------------------------------
    ...
    Arguments: (in calling order)
      Type       Name        I/O     Description
      ----       ----        ---     -----------
      int32_t    fst_call     I      flag to indicate if it is a first call
                                        or not for the given product file
      int32_t    fid          I      HDF file ID
      int32_t    sdfid        I      HDF ID
      int32_t    prod_ID      I      ID number of the given product
      int32_t    parm_opt     I      option flag array of parameters to be read
      int32_t    sbin         I      start/first bin number to retrieve data
      int32_t    nrec         I      number of bins to retrieve
      int32_t    binlist_key  I      BinList access ID
      int32_t *  begin        I      list of first bin in file for each row
      int32_t *  extent       I      no. of bins written to file for each row
      int32_t *  p_vskeys     O      parameter vdatas access IDs
      int32_t *  last_bin     O      contains next bin no. to be processed
      int32_t *  binno        O      bin numbers
      int16_t *  nobs         O      number of observations of each bin
      int16_t *  time_rec     O      time record for each bin
      int16_t *  nscenes      O      no. of scenes contributing to each bin
      float *    weights      O      statistical weight for each bin
      int8_t *   sel_cat      O      selection category for each binno bin
      int16_t *  flags_set    O      flag field for each binno bin
      float *    l3b_data     O      sums and sumsquares of all the parameters
...
intn
#ifdef PROTOTYPE
out_data(int32_t fst_call, int32_t fid, int32_t sdfid, int32_t prod_ID, l3b_prod *parm_opt,
        int32_t sbin, int32_t nrec, int32_t binlist_key, int32_t *begin, int32_t *extent,
        int32_t *p_vskeys, int32_t *last_bin, int32_t *binno, int16_t *nobs,
        int16_t *time_rec, int16_t *nscenes, float *weights, int8_t *sel_cat,
        int32_t *flags_set, float *l3b_data)
#else
out_data(fst_call, fid, sdfid, prod_ID, parm_opt, sbin, nrec, binlist_key,
        begin, extent, p_vskeys, last_bin, binno, nobs, time_rec, nscenes,
        weights, sel_cat, flags_set, l3b_data)
int32_t fst_call, fid, sdfid, prod_ID, sbin, nrec, binlist_key, *p_vskeys;
int32_t *begin, *extent, *binno, *last_bin, *flags_set;
int16_t *nobs, *time_rec, *nscenes;
int8_t *sel_cat;
float *weights, *l3b_data;
l3b_prod *parm_opt;
#endif
{
    int32_t i, j, parm, k, done = 1, pr_extent = 0, loc_sbin = sbin;
    int32_t l3b_index, n_databins = 0, bins_infile, element_index, rd_flg;
    int32_t sum_extent = 0, attrnum;
    static int32_t start[MAX_IN], nelts[MAX_IN], eof[MAX_IN];
...
/*----------------------------------------------------------------------------
    ...
    Arguments: (in calling order)
      ...
      ----       ----        ---     -----------
      int32_t    prod_ID      I      ID number of the given product
      int32_t    binlist_key  I      BinList vdata access ID
      int32_t *  p_vskey      I      parameter vdatas access IDs
      int32_t    start        I      start element number to read
      int32_t    nelts        I      number of elements to read
      int32_t *  parm_opt     I      option flag array of parameters to be read
...
intn
#ifdef PROTOTYPE
rd_data(int32_t prod_ID, int32_t binlist_key, int32_t *p_vskey, int32_t start,
        int32_t nelts, l3b_prod *parm_opt)
#else
rd_data(prod_ID, binlist_key, p_vskey, start, nelts, parm_opt)
int32_t prod_ID, binlist_key, *p_vskey, start, nelts;
l3b_prod *parm_opt;
#endif
{
    int32_t i, ret;
...
/*----------------------------------------------------------------------------
    ...
    Arguments: (in calling order)
      ...
      int32_t    parm         I      parameter number
      int32_t *  p_vskeys     O      parameter vdatas access IDs
...
intn
#ifdef PROTOTYPE
attach_slave(int32_t fid, char *sname, int32_t parm, int32_t *p_vskeys)
#else
attach_slave(fid, sname, parm, p_vskeys)
int32_t fid, *p_vskeys, parm;
char *sname;
#endif
{
    int32_t vsid;
    int32_t nelements, interlace, vsize;
...
/*-----------------------------------------------------------------------------
    ...
    Arguments: (in calling order)
      Type       Name        I/O     Description
      ----       ----        ---     -----------
      int32_t    prod_ID      I      ID number of the product
      int32_t    nbins        I      number of bins
...
intn
#ifdef PROTOTYPE
alloc_parm_bufs(int32_t prod_ID, int32_t nbins)
#else
alloc_parm_bufs(prod_ID, nbins)
int32_t prod_ID, nbins;
#endif
{
    int32_t i;

    if ((binno_list[prod_ID] = (int32_t *) malloc(nbins * sizeof (int32_t))) == NULL)
        return FAIL;

    if ((nobs_list[prod_ID] = (int16_t *) malloc(nbins * sizeof (int16_t))) == NULL)
        return FAIL;

    if ((nscenes_list[prod_ID] = (int16_t *) malloc(nbins * sizeof (int16_t))) == NULL)
        return FAIL;

    if ((timerec_list[prod_ID] = (int16_t *) malloc(nbins * sizeof (int16_t))) == NULL)
        return FAIL;

    if ((weights_list[prod_ID] = (float *) malloc(nbins * sizeof (float))) == NULL)
        return FAIL;

    if ((selcat_list[prod_ID] = (int8_t *) malloc(nbins * sizeof (int8_t))) == NULL)
        return FAIL;

    if ((flags_list[prod_ID] = (int32_t *) malloc(nbins * sizeof (int32_t))) == NULL)
        return FAIL;

    for (i = 0; i < NPARMS; i++) {
        if ((p[prod_ID][i] = (float *) malloc(2 * nbins * sizeof (float))) == NULL)
            return FAIL;
...
intn
#ifdef PROTOTYPE
free_parm_bufs(int32_t prod_ID)
#else
...
intn
#ifdef PROTOTYPE
get_index(int32_t sbin, int32_t prod_ID, int32_t bufsz)
#else
get_index(sbin, prod_ID, bufsz)
int32_t sbin, prod_ID, bufsz;
#endif
{

    int32_t i, done = 0, bufst, bufend, bufmid, element_index;
