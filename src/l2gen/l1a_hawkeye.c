...
static unsigned short **dn;                   // [num_bands][num_pixels], This scan, before cal
...
double ***alloc3d_double_h(size_t m, size_t n, size_t o) {
    // allocate values in one contiguous block
    double* p = (double*) malloc(m * n * o * sizeof(double));

    // allocate array for first dimension, size of m
    double*** array = (double***) malloc(m * sizeof(double **));

    for (size_t i = 0; i < m; i++) {
        // allocate array for second dimension, size of n
...
void free3d_double_h(double ***p, size_t m, size_t n, size_t o) {
...
int readl1a_hawkeye(filehandle *file, int32_t oline, l1str *l1rec) {
    ...
    for (int32 ip = 0; ip < num_pixels; ip++) {
        l1rec->pixnum[ip] = ip;
    }
    ...
    if (firstCall) {
        ...
        dn = (unsigned short**) allocate2d_short(num_bands, num_pixels);
    ...
    int32 line = oline + skip_beglines;
        ...
        if ((-1 < inputline) && (inputline < firstbad)) {
            ...
            for (pixel_num = 0; pixel_num < num_pixels; pixel_num++) {
                ...
                if ((-1 < ipix) && (ceil(inputpix) < num_pixels)) {
                    double Lt_interp = Lt[band_num][ipix] * (1 - fpix);
                    if (fpix > 0)
    ...
    if (file->geofile[0] == '\0') {
        ...
        // Fudge lat/lon
        for (pixel_num = 0; pixel_num < num_pixels; pixel_num++) {
            l1rec->lat[pixel_num] = lat + 5 * ((float) line / (float) num_scans);
            l1rec->lon[pixel_num] = lon + 5 * ((float) pixel_num / (float) num_pixels);
        }
...
int closel1a_hawkeye(filehandle *file) {
    ...
    if (file->geofile[0] != '\0') {
        ...
        if (K1) free(K1);
        if (K2) free3d_double_h(K2, num_bands, num_pixels, cal_num_dates);
...
void read_cal_hawkeye(char *cal_path) {
    ...
    // 3D arrays
    K2 = alloc3d_double_h(num_bands, num_pixels, cal_num_dates);
...
int calibrate_hawkeye(double current_julian_date) {
    ...
    // Misc declarations
    int band_num, pixel_num, ccd_temp_num;
    ...
    for (band_num = 0; band_num < num_bands; band_num++) {
        for (pixel_num = 0; pixel_num < num_pixels; pixel_num++) {
            this_dn = (double) dn[band_num][pixel_num] - bkg_avg[band_num][pixel_num];
            /* if (this_dn < (-1 * bkg_avg[band_num][pixel_num])) */
            /*     printf("B%d\tP%d\tdn - bkg_avg = %f\n", band_num, pixel_num, this_dn); */
            if (this_dn < 0)
                this_dn = 0;
            cal_coeff_1_4 = (K1[band_num]) * (K2i[band_num][pixel_num]) * (K3i[band_num])
                    * (K4[band_num][pixel_num]);

            // apply smile correction
            if (input->rad_opt)
                cal_coeff_1_4 *= K7[band_num][pixel_num];

            Lt[band_num][pixel_num] = this_dn * cal_coeff_1_4
