...
int read_target_l3(filehandle *file, l1str *l1rec, int32_t nbands, tgstr *tgrec) {
    ...
    if (firstCall) {
        ...
        read_l3bin(file->name, &l3bin, l1rec->l1file->nbands);
        nwaves = l3bin.nwave;
        ...
        for (ib = 0; ib < l1rec->l1file->nbands; ib++) {
            wvlmatch[ib] = windex(l1rec->l1file->fwave[ib], l3wvls, nwaves);
    ...
    for (ip = 0; ip < l1rec->npix; ip++) {
        ...
        if (input->vcal_depth < 0) {
            if (l1rec->elev[ip] > input->vcal_depth)
                continue;
        }
        // locate bin 
        ...
        if (idx >= 0) {
            ...
            if (chl <= input->chlthreshold && aot <= input->aotthreshold
                ...
                for (ib = 0; ib < nvisbands; ib++) {
                    ...
                    if (needBandShift && (l3wvls[wvlmatch[ib]] != l1rec->l1file->fwave[ib])) {
                        ...
                        if (input->band_shift_opt == 0 || l1rec->l1file->fwave[ib] > 700)
                            interpband = linterp(l3wvls, l3vals, nwaves, l1rec->l1file->fwave[ib]);

                            // EK: option 1: bio-optical band shift. added second condition
                        else if (input->band_shift_opt == 1 && l1rec->l1file->fwave[ib] <= 700)
                            interpband = bioBandShift(l3wvls, l3vals, chl, nwaves, l1rec->l1file->fwave[ib]);
