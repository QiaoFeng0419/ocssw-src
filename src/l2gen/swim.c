...
void flag_shallow(l2str *l2rec, double *rrsSub, double *rrsAbove) {
    ...
    double od547;
    int ntwave = l2rec->l1rec->l1file->nbands;
...
void run_swim(l2str *l2rec) {
    ...
    if (swimScanNum == -1) {
        ...
        kdtot = (float*) allocateMemory(l1rec->npix * nbandVis * sizeof (float),
                "kdtot");
        zirrad = (float*) allocateMemory(l1rec->npix * nbandVis * sizeof (float),
                "zirrad");
        chla = (float*) allocateMemory(l1rec->npix * sizeof (float), "chla");
        tss = (float*) allocateMemory(l1rec->npix * sizeof (float), "tss");
        ...
        if (l1rec->mask[i] == 0) {
            ...
            if (statusLM <= 0) {
                ...
                for (iw = 0; iw < nbandVis; iw++) {
                    ...
                    if (l2rec->La[i * l1file->nbands + iw] > 0.0) {
                        ed = l1rec->Fo[iw] * l1rec->tg_sol[i * l1file->nbands + iw]
                                * l1rec->t_sol[i * l1file->nbands + iw]
                                * cos(l1rec->solz[i] * (3.141592654 / 180.));
                    } else {
                        ed = 0.0;
                    }
                    //compute irradiance at depth
                    zirrad[i * nbandVis + iw] = ed
                            * exp(l1rec->elev[i] * kdcoeff);
