...
static int32 geoFileId;
static int32 lat_id;
static int32 lon_id;

static int32 sd_id[N_CAMERAS];
static int32 red_id[N_CAMERAS];
static int32 grn_id[N_CAMERAS];
static int32 blu_id[N_CAMERAS];

static int32 nir_id[N_CAMERAS];
...
int openl1b_misr(filehandle *file) {
    ...
    misr_t *private_data = (misr_t *) calloc(1, sizeof(misr_t));

    file->private_data = private_data;

    for (i=0; i<N_CAMERAS; i++) sd_id[i] = -1;
                                         
    if ( strncmp(basename(file->name)+39, "XX", 2) == 0) {
    ...
    for (i=0; i<180; i++) {
      //      if ( private_data->isOceanBlock[i] == 1) {
...
int getRadScaleFactors( char *file, double rad_scl_factors[4]) {
  ...
  int i,j;
  int32 file_id, vg_band_ref, refn, vg_id, vd_id;
  int32 nObj, tag, ref;
  char name[100];
...
int readl1b_misr(filehandle *l1file, l1str *l1rec) {
  ...
  int32 start[3]={0,0,0};
  int32 count[3]={1,1,512};
  
  //static int32 last_recnum=-1;
  static int32 last_recnum_gp=-1;

  //int32 diff_offset, interp_index;
  int32 recnum, recnum_red, recnum_gp, block;
...
//    }
    ...
    for (j=0; j<N_CAMERAS; j++) {
      ...
      for (i=0; i<32; i++) {
        dbl_data[i]    = private_data->SenAzimuth[j][recnum_gp+0][i];
        dbl_data[i+32] = private_data->SenAzimuth[j][recnum_gp+1][i];
      }
      interp_values( dbl_data, sna_interp_data[j]);
    }
...
int interp_values( double *grid_values, double interpolated_values[16][512]) {
  ...
  for (size_t j=0; j<16; ++j) {
    ...
    for (size_t i=8; i<504; ++i) {
      double xi = (double) i;
      //printf("%d %f %d %f\n", j, yi, i, xi);
      if (grid_values[(i-8)/16] < 0 || grid_values[(i-8)/16+1] < 0 ||
          grid_values[(i-8)/16+32] < 0 || grid_values[(i-8)/16+32+1] < 0)
