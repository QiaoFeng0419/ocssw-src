#ifndef OC_STRINGUTILS
#define OC_STRINGUTILS
...
class StringUtils {
    public:
        ...
        */
        template<typename T>
        static std::vector<T> stov(std::string& str, const std::string& delims=",");
        ...
        */
        template<typename T>
        static std::vector<T> stov(std::string& str, const std::string& delims, std::function<T(const std::string&)> parser);
    private:
