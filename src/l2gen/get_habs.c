...
void habs_meris_ci_corr(float rhos443, float rhos490, float rhos560,
        float rhos620, float rhos665, float rhos709, float rhos865, float *ci) {

    float kd, kd_709, ss_560;

    kd = (0.7 * ((((rhos620 + rhos665) / 2.) - rhos865) / (((rhos443 + rhos490) / 2.) - rhos865)));
    //apply calibration
    kd = (4.0 * kd) - 0.69;

    //calculate Kd using rhos_709 instead of rhos_620 & rhos_665
    kd_709 = (0.7 * ((rhos709 - rhos865) / (((rhos443 + rhos490) / 2.) - rhos865)));
    //apply calibration
    kd_709 = (4.0 * kd_709) - 0.69;
    ...
    if (((((rhos620 + rhos665) / 2.) - rhos865) > 0) &&
            ((((rhos443 + rhos490) / 2.) - rhos865) > 0)) {
        if ((kd < 0.31) &&
            ((rhos865 <= rhos490) ||
            (rhos865 <= rhos665) ||
            (rhos865 <= rhos709)) &&
...
void get_habs_ci(l2str *l2rec, l2prodstr *p, float ci[]) {
    ...
    float wav0, wav1, wav2, wav3, wav4, wav5, wav6, wav7, nonci, fac;
    ...
    for (ip = 0; ip < l2rec->l1rec->npix; ip++) {
        ...
        if ((l2rec->l1rec->height[ip] == 0 && l2rec->l1rec->elev[ip] < 0) ||
            ...
            case CAT_CI_noncyano:
                ...
                if (l2rec->l1rec->l1file->sensorID == MERIS || l2rec->l1rec->l1file->sensorID == OLCIS3A 
                    ...
                    habs_meris_ci_corr(l2rec->l1rec->rhos[ipb + ib4], l2rec->l1rec->rhos[ipb + ib5],
                                       ...
                                       l2rec->l1rec->rhos[ipb + ib7], &ci[ip]);

                    if (p->cat_ix == CAT_CI_cyano || p->cat_ix == CAT_CI_noncyano) {
                        nonci = l2rec->l1rec->rhos[ipb + ib1]
