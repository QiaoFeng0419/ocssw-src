...
int lonlat2pixline(lonlat2pixline_t *params) {
    ...
    int32 sd_id;    // HDF4 params for MODISGEO
    int32 sds_id_ll[2];
    int32 start[3] = {0, 0, 0};
    int32 edges[3];
    ...
    type = getFormatType(l1file->name);

    if (type == FT_MODISL1B && (params->resolution == 500 || params->resolution == 250)) {
    ...
    } else if (type == FT_VIIRSGEONC) {
        int dimids[] = {-1,-1};
        size_t dimlen;

        status = nc_open(params->input_filename, NC_NOWRITE, &ncid);
        if (status != NC_NOERR) {
            fprintf(stderr,
