...
static int32_t numNoiseScales = -1;
static int32_t numBiasFracs = -1;
instr *input;
...
void msl12_input_nbands_init(instr *input, int32_t nbands) {
    ...
    input->xcal_opt = calloc_nbandsi32t(nbands, input->xcal_opt, 0);
    input->xcal_wave = calloc_nbandsf(nbands, input->xcal_wave, -1.0);
    input->gain = calloc_nbandsf(nbands, input->gain, 1.0);
    input->gain_unc = calloc_nbandsf(nbands, input->gain_unc, 0.0);
    input->offset = calloc_nbandsf(nbands, input->offset, 0.0);
    ...
    input->lt_noise_scale = calloc_nbandsf(nbands, input->lt_noise_scale, 1.0);
    input->bias_frac = calloc_nbandsf(nbands, input->bias_frac, 0.0);
...
void msl12_input_init() {
    ...
    input->add_lt_noise = 0.0;
    input->add_ws_noise = -1.0;
    input->add_wd_noise = -1.0;
    input->add_mw_noise = -1.0;
    input->add_zw_noise = -1.0;
    input->add_rh_noise = -1.0;
    input->add_pr_noise = -1.0;
    input->add_wv_noise = -1.0;
    input->add_oz_noise = -1.0;
    input->add_no2_strat_noise = -1.0;
    input->add_no2_tropo_noise = -1.0;
...
int l2gen_init_options(clo_optionList_t* list, const char* prog) {
    ...
    strncpy(tmpStr1, tmpStr, 2048);
    sprintf(tmpStr, "%s           and MUMM NIR calculation", tmpStr1);
//    strncpy(tmpStr1, tmpStr, 2048);
...
//    sprintf(tmpStr, "%s      %3d: Spectral matching via Gordon (SeaWiFS-only)", tmpStr1, AERSMA);
    ...
    strcpy(tmpStr, "wind speed uncertainty simulation option \n");
    strcat(tmpStr, "       <0.0: no noise added\n");
    strcat(tmpStr, "       =0.0: use ws_unc to inform noise model\n");
    strcat(tmpStr, "       >0.0: use input to inform noise model");
    clo_addOption(list, "add_ws_noise", CLO_TYPE_FLOAT, "-1.0", tmpStr);

    strcpy(tmpStr, "wind direction uncertainty simulation option \n");
    strcat(tmpStr, "       <0.0: no noise added\n");
    strcat(tmpStr, "       =0.0: use wd_unc to inform noise model\n");
    strcat(tmpStr, "       >0.0: use input to inform noise model");
    clo_addOption(list, "add_wd_noise", CLO_TYPE_FLOAT, "-1.0", tmpStr);

    strcpy(tmpStr, "meridional wind  uncertainty simulation option \n");
    strcat(tmpStr, "       <0.0: no noise added\n");
    strcat(tmpStr, "       =0.0: use mw_unc to inform noise model\n");
    strcat(tmpStr, "       >0.0: use input to inform noise model");
    clo_addOption(list, "add_mw_noise", CLO_TYPE_FLOAT, "-1.0", tmpStr);

    strcpy(tmpStr, "zonal wind uncertainty simulation option \n");
    strcat(tmpStr, "       <0.0: no noise added\n");
    strcat(tmpStr, "       =0.0: use zw_unc to inform noise model\n");
    strcat(tmpStr, "       >0.0: use input to inform noise model");
    clo_addOption(list, "add_zw_noise", CLO_TYPE_FLOAT, "-1.0", tmpStr);

    strcpy(tmpStr, "relative humidity uncertainty simulation option \n");
    strcat(tmpStr, "       <0.0: no noise added\n");
    strcat(tmpStr, "       =0.0: use rh_unc to inform noise model\n");
    strcat(tmpStr, "       >0.0: use input to inform noise model");
    clo_addOption(list, "add_rh_noise", CLO_TYPE_FLOAT, "-1.0", tmpStr);

    strcpy(tmpStr, "pressure uncertainty simulation option \n");
    strcat(tmpStr, "       <0.0: no noise added\n");
    strcat(tmpStr, "       =0.0: use pr_unc to inform noise model\n");
    strcat(tmpStr, "       >0.0: use input to inform noise model");
    clo_addOption(list, "add_pr_noise", CLO_TYPE_FLOAT, "-1.0", tmpStr);

    strcpy(tmpStr, "water vapor uncertainty simulation option \n");
    strcat(tmpStr, "       <0.0: no noise added\n");
    strcat(tmpStr, "       =0.0: use wv_unc to inform noise model\n");
    strcat(tmpStr, "       >0.0: use input to inform noise model");
    clo_addOption(list, "add_wv_noise", CLO_TYPE_FLOAT, "-1.0", tmpStr);

    strcpy(tmpStr, "ozone uncertainty simulation option \n");
    strcat(tmpStr, "       <0.0: no noise added\n");
    strcat(tmpStr, "       =0.0: use oz_unc to inform noise model\n");
    strcat(tmpStr, "       >0.0: use input to inform noise model");
    clo_addOption(list, "add_oz_noise", CLO_TYPE_FLOAT, "-1.0", tmpStr);

    strcpy(tmpStr, "tropospheric no2 uncertainty simulation option \n");
    strcat(tmpStr, "       <0.0: no noise added\n");
    strcat(tmpStr, "       =0.0: use no2_tropo_unc to inform noise model\n");
    strcat(tmpStr, "       >0.0: use input to inform noise model");
    clo_addOption(list, "add_no2_tropo_noise", CLO_TYPE_FLOAT, "-1.0", tmpStr);

    strcpy(tmpStr, "stratospheric no2 uncertainty simulation option \n");
    strcat(tmpStr, "       <0.0: no noise added\n");
    strcat(tmpStr, "       =0.0: use no2_strat_unc to inform noise model\n");
    strcat(tmpStr, "       >0.0: use input to inform noise model");
    clo_addOption(list, "add_no2_strat_noise", CLO_TYPE_FLOAT, "-1.0", tmpStr);

    strcpy(tmpStr, "Lt uncertainty simulation option \n");
    strcat(tmpStr, "       >0.0: add random normal noise to gain\n");
    strcat(tmpStr, "        0.0: no random noise added");
    clo_addOption(list, "add_lt_noise", CLO_TYPE_FLOAT, "0.0", tmpStr);

    strcpy(tmpStr, "Lt noise scale factor option \n");
    strcat(tmpStr, "       !=1.0: scales noise by factoring SNR\n");
    strcat(tmpStr, "       =1.0: noise not scaled");
    clo_addOption(list, "lt_noise_scale", CLO_TYPE_FLOAT, NULL, tmpStr);

    strcpy(tmpStr, "bias fraction factor option \n");
    strcat(tmpStr, "       !=0.0: adds bias as a fraction of Lt\n");
    strcat(tmpStr, "       =0.0: no bias added");
    clo_addOption(list, "bias_frac", CLO_TYPE_FLOAT, NULL, tmpStr);
    ...
    clo_addOption(list, "demfile", CLO_TYPE_IFILE, "$OCDATAROOT/common/ETOPO1_ocssw.nc", "\n        digital elevation map file");
...
int l2gen_read_options(clo_optionList_t* list, char* progName,
    ...
    sprintf(tmpStr, "%s/%s/%s_defaults.par", dataRoot,
            sensorId2SensorDir(l1file->sensorID), l2genProgName);
    if (want_verbose)
        printf("Loading default parameters for %s from %s\n",
            sensorId2SensorName(l1file->sensorID), tmpStr);
    clo_readFile(list, tmpStr);

    // load the sensor specific other program defaults file
    if (otherProgName) {
        sprintf(tmpStr, "%s/%s/%s_defaults.par", dataRoot,
    ...
    if (l1file->subsensorID >= 0) {
        sprintf(tmpStr, "%s/%s/%s/%s_defaults.par", dataRoot,
                ...
                subsensorId2SubsensorDir(l1file->subsensorID), l2genProgName);
        if (access(tmpStr, R_OK) != -1) {
            if (want_verbose)
...
int l2gen_load_input(clo_optionList_t *list, instr *input, int32_t nbands) {
    ...
    for (optionId = 0; optionId < numOptions; optionId++) {
        ...
        if (strcmp(keyword, "help") == 0)
        ...
        } else if (strcmp(keyword, "add_ws_noise") == 0) {
            input->add_ws_noise = clo_getOptionFloat(option);
        } else if (strcmp(keyword, "add_wd_noise") == 0) {
            input->add_wd_noise = clo_getOptionFloat(option);
        } else if (strcmp(keyword, "add_mw_noise") == 0) {
            input->add_mw_noise = clo_getOptionFloat(option);
        } else if (strcmp(keyword, "add_zw_noise") == 0) {
            input->add_zw_noise = clo_getOptionFloat(option);
        } else if (strcmp(keyword, "add_pr_noise") == 0) {
            input->add_pr_noise = clo_getOptionFloat(option);
        } else if (strcmp(keyword, "add_rh_noise") == 0) {
            input->add_rh_noise = clo_getOptionFloat(option);
        } else if (strcmp(keyword, "add_wv_noise") == 0) {
            input->add_wv_noise = clo_getOptionFloat(option);
        } else if (strcmp(keyword, "add_oz_noise") == 0) {
            input->add_oz_noise = clo_getOptionFloat(option);
        } else if (strcmp(keyword, "add_no2_strat_noise") == 0) {
            input->add_no2_strat_noise = clo_getOptionFloat(option);
        } else if (strcmp(keyword, "add_no2_tropo_noise") == 0) {
            input->add_no2_tropo_noise = clo_getOptionFloat(option);
        } else if (strcmp(keyword, "add_lt_noise") == 0) {
            input->add_lt_noise = clo_getOptionFloat(option);
        } else if (strcmp(keyword, "lt_noise_scale") == 0) {
            if (clo_isOptionSet(option)) {
                fArray = clo_getOptionFloats(option, &count);
                if (count > nbands) {
                    printf("-E- number of lt_noise_scale elements (%d) must be %d or less\n", count, nbands);
                    exit(1);
                }
                for (i = 0; i < count; i++)
                    input->lt_noise_scale[i] = fArray[i];
                numNoiseScales = count;
            }
        } else if (strcmp(keyword, "bias_frac") == 0) {
            if (clo_isOptionSet(option)) {
                fArray = clo_getOptionFloats(option, &count);
                if (count > nbands) {
                    printf("-E- number of bias_frac elements (%d) must be %d or less\n", count, nbands);
                    exit(1);
                }
                for (i = 0; i < count; i++)
                    input->bias_frac[i] = fArray[i];
                numBiasFracs = count;
            }
            ...
            input->geom_per_band = clo_getOptionBool(option);
...
int msl12_option_input(int argc, char **argv, clo_optionList_t* list,
    ...
    if (numNoiseScales > 0 && numNoiseScales != numBands) {
        fprintf(stderr, "Parameter input error:  Number of input gain uncertainties must equal %d.\n",
                numBands);
        return (-1);
    }
    if (numBiasFracs > 0 && numBiasFracs != numBands) {
        fprintf(stderr, "Parameter input error:  Number of bias fractions must equal %d.\n",
                numBands);
        return (-1);
    }
    ...
    sprintf(str_buf, "geom_per_band = %3d", input->geom_per_band);
    strcat(input->input_parms, str_buf);
    strcat(input->input_parms, "\n");
    ...
    sprintf(str_buf, "add_ws_noise = %2.4f", input->add_ws_noise);
    strcat(input->input_parms, str_buf);
    strcat(input->input_parms, "\n");

    sprintf(str_buf, "add_wd_noise = %2.4f", input->add_wd_noise);
    strcat(input->input_parms, str_buf);
    strcat(input->input_parms, "\n");

    sprintf(str_buf, "add_mw_noise = %2.4f", input->add_mw_noise);
    strcat(input->input_parms, str_buf);
    strcat(input->input_parms, "\n");

    sprintf(str_buf, "add_zw_noise = %2.4f", input->add_zw_noise);
    strcat(input->input_parms, str_buf);
    strcat(input->input_parms, "\n");

    sprintf(str_buf, "add_rh_noise = %2.4f", input->add_rh_noise);
    strcat(input->input_parms, str_buf);
    strcat(input->input_parms, "\n");

    sprintf(str_buf, "add_pr_noise = %2.4f", input->add_pr_noise);
    strcat(input->input_parms, str_buf);
    strcat(input->input_parms, "\n");

    sprintf(str_buf, "add_wv_noise = %2.4f", input->add_wv_noise);
    strcat(input->input_parms, str_buf);
    strcat(input->input_parms, "\n");

    sprintf(str_buf, "add_oz_noise = %2.4f", input->add_oz_noise);
    strcat(input->input_parms, str_buf);
    strcat(input->input_parms, "\n");

    sprintf(str_buf, "add_no2_strat_noise = %2.4f", input->add_no2_strat_noise);
    strcat(input->input_parms, str_buf);
    strcat(input->input_parms, "\n");

    sprintf(str_buf, "add_no2_tropo_noise = %2.4f", input->add_no2_tropo_noise);
    strcat(input->input_parms, str_buf);
    strcat(input->input_parms, "\n");

    sprintf(str_buf, "add_lt_noise = %2.4f", input->add_lt_noise);
    strcat(input->input_parms, str_buf);
    strcat(input->input_parms, "\n");

    sprintf(str_buf, "lt_noise_scale = %2.4f", input->lt_noise_scale[0]);
    strcat(input->input_parms, str_buf);
    for (i = 1; i < numBands; i++) {
        sprintf(str_buf, ", %2.4f", input->lt_noise_scale[i]);
        strcat(input->input_parms, str_buf);
    }
    strcat(input->input_parms, "\n");

    sprintf(str_buf, "bias_frac = %2.4f", input->bias_frac[0]);
    strcat(input->input_parms, str_buf);
    for (i = 1; i < numBands; i++) {
        sprintf(str_buf, ", %2.4f", input->bias_frac[i]);
        strcat(input->input_parms, str_buf);
    }
    strcat(input->input_parms, "\n");
