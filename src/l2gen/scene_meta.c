...
void scene_meta_put(l1str *l1rec) {
    ...
    if (firstCall) {
        ...
        scene_meta_init();
        if (input->eline < 1)
            cscan = ((l1rec->l1file->nscan - 1) - MAX(input->sline - 1, 0)) / 2;
        else
            cscan = ((input->eline - 1) - MAX(input->sline - 1, 0)) / 2;
    }

    /* ignore buffering scans */
    if (l1rec->iscan == lastScan) return;
    if (l1rec->iscan < MAX(input->sline - 1, 0)) return;
    if (input->eline < 1) {
        if (l1rec->iscan > (l1rec->l1file->nscan - 1))
            return;
    } else {
        if (l1rec->iscan > (input->eline - 1))
            return;
    }
