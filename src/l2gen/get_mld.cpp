...
float get_mld(char* mldfile, float lon, float lat, int day) {
    ...
    if (firstCall) {
        ...
        netcdf_mld = (mldfile != NULL && mldfile[0] != 0);

        // is it valid format?
        if (netcdf_mld) {
            mld = get_mld_nc(mldfile, lon, lat);  // good file?
            netcdf_mld = (mld != BAD_FLT);
            if (netcdf_mld) {
                printf("Opening MLD climatology file %s\n", mldfile);
            } else {
                printf("Specified MLD file %s does not have correct format;\n", mldfile);
                printf("will read MLD info from default climatology.\n");
